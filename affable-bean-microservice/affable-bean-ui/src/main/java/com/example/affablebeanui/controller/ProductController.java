...
public class ProductController {
    ...
    public String showCheckoutInfo(Model model){
        var response = productService.getOrderProductInfo(orderId);
        var totalPrice = response.products()
                        .stream().map( p -> p.price() * p.quantity())
                        .mapToDouble(Double::doubleValue).sum();
        var deliveryCharge = response.totalAmount() -totalPrice;
        var deliveryChargeFloat = String.valueOf(deliveryCharge).formatted("%.2f");
        model.addAttribute("deliveryCharge",deliveryChargeFloat);
        model.addAttribute("orderProduct",response);
         model.addAttribute("orderId",orderId);
         model.addAttribute("totalPrice",response.totalAmount());
         return "checkoutInfo";
    }
